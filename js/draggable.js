 ...
 {const theDragElement = config.hasOwnProperty("dragElement")?element.querySelector(config.dragElement):element;
  // console.log("element",element,"theDragElement",theDragElement,"qs",element.querySelector(config.dragElement));
		...
		if(getComputedStyle(theDragElement).position == "static" ){element.style.position="relative";/* need this so newDiv is correctly sized and positioned. */
																  element.dataset.changedByScript = "changed to relative by KTWP DE #1"; /* for troubleshooting */}
 }
...
class AdvancedDraggable {
    constructor(config) {
        ...
        // Cache for drag calculations
        this.dragCache = {};

        // --- Touch Movement Tracking ---
        this.touchStartX = 0;
        this.touchStartY = 0;
        this.touchMovementThreshold = 10;
        this.isTouchSequenceActive = false;
    ...
    init() {
        const performSetup = () => this.setupDraggables();
            ...
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    ...
                    if (mutation.type === 'childList') {
                        ...
                        theseNodes = [mutation.target];
                    }

                    if (theseNodes.length === 0) return;

                    // MutationObserver usually handles small batches, so standard one-by-one is okay here,
                    // but we will use the same 'makeDraggable' logic which now supports pre-calculated data.
                    theseNodes.forEach((node) => {
                        if (node.nodeType === 1) { 
                            this.config.forEach(item => {
                                if (node.matches && node.matches(item.selector)) {
    ...
    setupDraggables() {
        if (!this.stylesInjected) {
            ...
            *[data-draggable=true] .ktwp-de-effectsDiv::before {
                z-index:999999; opacity: 0; transition: opacity .1s ease-in; transition-delay: 0s; content: ''; position: absolute; top: -40%; left: -40%; right: -40%; bottom: -40%; background-repeat: no-repeat; background-position: center center; background-size: clamp(60px,50%,160px); pointer-events: none; overflow: visible;
                background-image: url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\"><path stroke=\"rgba(190,190,190,1)\" stroke-width=\".7\"  fill=\"rgba(0,0,0,0.3)\" d=\"M8.326 5.327 12.03 1l3.705 4.327h-1.853v4.349h-3.705V5.326zM15.736 18.734 12.03 23.06l-3.706-4.327h1.853v-4.348h3.705v4.348zM18.734 8.325l4.327 3.706-4.327 3.705v-1.853h-4.348v-3.705h4.348zM5.327 15.736 1 12.03l4.327-3.706v1.853h4.349v3.705H5.326z\"/></svg>');
            }
            .ktwp-de-effectsDiv{position:absolute !important; left:0 !important;top:0 !important;bottom:0 !important;right:0 !important;background:transparent !important;filter:none !important;backdropFilter:none !important;border-radius:inherit;pointer-events:none;}
            .is-dragging .ktwp-de-effectsDiv::before { opacity: .5; }
            *[data-draggable="true"]:hover:not(.is-dragging):not(.ktwp-de-disablehover) .ktwp-de-effectsDiv::before { animation: animateArrows 2s linear; animation-delay:1.5s; }
            *[data-draggable=true].ktwp-de-cdrag .ktwp-de-effectsDiv::before {background-image: url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\"><g fill=\"rgba(0,0,0,0.3)\" stroke=\"rgba(190,190,190,1)\" stroke-width=\".7\" ><path  d=\"m5.56 15.5-2.74 3.201-2.74-3.2h1.37v-3.217h2.74V15.5zM18.5 8.828l2.741-3.2 2.74 3.2h-1.37v3.216h-2.74V8.828zM.08 9.056l2.74-3.201 2.74 3.2H4.19v3.217H1.45V9.056zM23.981 15.273l-2.74 3.201-2.74-3.2h1.37v-3.217h2.74v3.216z\"/><circle cx=\"2.77\" cy=\"3.163\" r=\"1.993\"/><circle cx=\"2.77\" cy=\"21.389\" r=\"1.993\"/><circle cx=\"21.189\" cy=\"3.163\" r=\"1.993\"/><circle cx=\"21.189\" cy=\"21.389\" r=\"1.993\"/><circle cx=\"50.136\" cy=\"20.534\" r=\"0\"/><path d=\"m8.688 5.694-3.2-2.74 3.2-2.74v1.37h3.216v2.74H8.688zM15.133.214l3.2 2.74-3.2 2.74v-1.37h-3.216v-2.74h3.216zM15.133 18.634l3.2 2.74-3.2 2.741v-1.37h-3.216v-2.74h3.216zM8.688 24.115l-3.2-2.74 3.2-2.74v1.37h3.216v2.74H8.688z\"/></g></svg>');}
            *[data-draggable=true].ktwp-de-hdrag > .ktwp-de-effectsDiv::before {background-image: url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\"><path stroke=\"rgba(190,190,190,1)\" stroke-width=\".7\"  fill=\"rgba(0,0,0,0.3)\" d=\"m18.734 8.325 4.327 3.706-4.327 3.705v-1.853h-4.348v-3.705h4.348zM5.327 15.736 1 12.03l4.327-3.706v1.853h4.349v3.705H5.326z\"/></svg>');}
            *[data-draggable=true].ktwp-de-vdrag > .ktwp-de-effectsDiv::before {background-image: url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\"><path stroke=\"rgba(190,190,190,1)\" stroke-width=\".7\"  fill=\"rgba(0,0,0,0.3)\" d=\"M8.326 5.327 12.03 1l3.705 4.327h-1.853v4.349h-3.705V5.326zM15.736 18.734 12.03 23.06l-3.706-4.327h1.853v-4.348h3.705v4.348z\"/></svg>');}
            *[data-draggable=true]  .ktwp-de-effectsDiv::after {
                opacity: 0; transition: opacity .1s ease-in; transition-delay: 0s; content: ''; position: absolute; top: -2px; left: -2px; right: -2px; bottom: -2px; border-radius: inherit; position: absolute; filter: blur(5px);
                background: linear-gradient(to left, red, orange, yellow, green, blue, #8800FF, #FF0088);
                animation-delay: 3s; background-size: 200% 200%; animation: animateGlow 1.25s linear infinite;
                clip-path: polygon(-100% -100%, -100% 200%, -1% 200%, -1% -1%, 101% -1%, 101% 101%, -1% 101%, -1% 200%, 200% 200%, 200% -100%);
            }
            *[data-draggable="true"]:not(.is-dragging):not(.ktwp-de-disablehover):hover  .ktwp-de-effectsDiv::after { opacity: .8; transition-delay: 2.5s; transition-duration: 0.5s; }
            .is-dragging > .ktwp-de-effectsDiv::after { opacity: .8; transition-delay: 0s; transition-duration: 0s; }
            @keyframes animateGlow { 0% { background-position: 0% 50%; } 100% { background-position: 200% 50%; } }
            @keyframes animateArrows { 0%,25%,50%,75%,100% { opacity: 0; } 12%,37%,62%, 83% { opacity: .98; } }
            .is-dragging  > .ktwp-de-effectsDiv { cursor: grabbing !important; border-width:2px; border:inherit dashed  #777 !important; border-color:rgba(0,0,0,0) !important; }
            `;
            var cssStyle = document.createElement("style");
            cssStyle.textContent = cssRules;
            document.head.appendChild(cssStyle);
        ...
        // --- PHASE 1: READ (Measure everything first) ---
        const initBatch = [];
        ...
        this.config.forEach(item => {
            ...
            elements.forEach(element => {
                if (!element.hasAttribute('data-draggable')) {
                    // Collect all necessary measurements here
                    const rect = element.getBoundingClientRect();
                    const compStyle = getComputedStyle(element);
                    
                    // Handle logic for checking the internal drag element's position
                    // We must do this read here to avoid thrashing later when we prepend the div
                    let needsRelativePos = false;
                    let targetDragElement = element;
                    if (item.hasOwnProperty("dragElement") && item.dragElement != '') {
                         targetDragElement = element.querySelector(item.dragElement) || element;
                         if (getComputedStyle(targetDragElement).position == "static") {
                             needsRelativePos = true;
                         }
                    } else {
                         // Default behavior if no specific drag element
                         if (compStyle.position == "static" && !item.dragElement) {
                             // Note: Original logic checked theDragElement. Here it's the element itself.
                             // Logic: If we prepend a div, we might need relative positioning.
                             // Preserving original logic's intent:
                             needsRelativePos = (compStyle.position == "static"); 
                         }
                    }
                    ...
                    initBatch.push({
                        ...
                        compStyle: {
                            ...
                            zIndex: compStyle.zIndex // if needed later
                        },
                        needsRelativePos: needsRelativePos,
                        targetDragElement: targetDragElement,
        ...
        // --- PHASE 2: WRITE (Apply changes based on measurements) ---
        initBatch.forEach(data => {
            this.makeDraggable(data.element, data.config, data);
        });
    }

    // Now accepts optional 'preCalculatedData' to skip DOM reads
    makeDraggable(element, config, preCalculatedData = null) {
        element.draggableConfig = config;
        
        // If data wasn't pre-calculated (e.g. from MutationObserver), read it now
        if (!preCalculatedData) {
            ...
            if (config.hasOwnProperty("dragElement") && config.dragElement != '') {
                targetDragElement = element.querySelector(config.dragElement) || element;
            }
            // Check for position fix logic (READ)
            if (getComputedStyle(targetDragElement).position == "static") {
                needsRelativePos = true;
            }
            ...
            preCalculatedData = {
                rect: rect,
                compStyle: { position: compStyle.position },
                needsRelativePos: needsRelativePos,
                targetDragElement: targetDragElement,
                scrollX: window.scrollX,
                scrollY: window.scrollY
        ...
        const { rect, compStyle, needsRelativePos, targetDragElement, scrollX, scrollY } = preCalculatedData;

        // Apply attributes (WRITE)
        element.setAttribute('ktwp-de-rect-height', rect.height);
        element.setAttribute('ktwp-de-rect-width', rect.width);
        
        const calcScrollX = compStyle.position === 'fixed' ? 0 : scrollX;
        const calcScrollY = compStyle.position === 'fixed' ? 0 : scrollY;
        
        element.setAttribute('ktwp-de-rect-x', rect.x + calcScrollX);
        element.setAttribute('ktwp-de-rect-y', rect.y + calcScrollY);
        element.setAttribute('data-draggable', 'true');
        element.setAttribute('data-ktwp-de-position', compStyle.position);
        ...
        const constraintDesc = {
            "vertical": { "desc": "vertically", "class": "vdrag" },
        ...
        if ((config.hasOwnProperty("dragElement") && config.dragElement != '') || !config.hasOwnProperty("dragElement")) {
            const newDiv = document.createElement("div");
            newDiv.className = "ktwp-de-effectsDiv";
            targetDragElement.prepend(newDiv); // WRITE (Layout Invalidation)
            
            // We used pre-calculated data here to avoid a Read-after-Write
            if (needsRelativePos) {
                element.style.position = "relative";
				element.dataset.changedByScript = "changed to relative by KTWP DE #2"; /* for troubleshooting */
            }
        }

        // Corner constraint pre-positioning
        if (config.constraint === 'corners') {
            const margin = config.cornerMargin || 25;
            element.style.position = 'fixed';
            element.style.left = margin + 'px';
            ...
            element.style.right = 'auto';
            element.style.zIndex = '9999';
            document.body.appendChild(element); // WRITE
        }
        ...
        if (element.id && sessionStorage.getItem("ktwp-de-elem-" + element.id + "-x")) {
            ...
            // We pass the pre-calculated rect to handleMouseDown to prevent it from reading the DOM again
            // during this "simulated" phase, avoiding further thrashing.
            this.handleMouseDown({
                button: 0,
                target: element,
                clientX: rect.left,
                clientY: rect.top,
                preventDefault: () => {}
            }, rect); // Pass rect as optional 2nd arg

            this.handleMouseUp({
                button: 0,
                target: element,
                clientX: parseFloat(absoluteX),
                clientY: parseFloat(absoluteY),
                preventDefault: () => {},
                simulated: true
            });
    ...
    // Updated handleMouseDown to accept optional pre-known rect (cachedRect)
    handleMouseDown(e, cachedRect = null) {
        if (e.button !== 0) return;
        e.preventDefault();
        ...
        // Use cachedRect if provided (for initialization/restoration), otherwise Read DOM
        const rect = cachedRect || element.getBoundingClientRect(); 
        
        // This must still be read unfortunately if not passed, but we can't cache computed styles forever.
        // For the restoration phase, we accept it.
        const computedStyle = getComputedStyle(element);

        // Populate Cache for the Drag Loop
        this.dragCache = {
        ...
        if (computedStyle.position != 'fixed' && computedStyle.position != 'absolute') {
            ...
            newNode.id = nodeUUID;
            newNode.style.margin = computedStyle.margin;
            newNode.style.top = rect.top + "px";
            newNode.style.left = rect.left + "px";
            newNode.style.height = rect.height + "px";
            newNode.style.width = rect.width + "px";
            newNode.style.opacity = '0';
            newNode.setAttribute('data-draggable-placeholder', 'true');
            element.setAttribute('ktwp-de-placeholderId', nodeUUID);
            element.parentNode.insertBefore(newNode, element);
        }

        if (newNode || (computedStyle.position == 'absolute')) {
    ...
    handleTouchMove(e) {
        ...
        if (!this.dragStarted) {
            if (deltaX > this.touchMovementThreshold || deltaY > this.touchMovementThreshold) {
                this.dragStarted = true;
                e.preventDefault();
                this.activeElement.classList.add("ktwp-de-disablehover");
            } else {
                return;
            }
        } else {
    ...
    updateElementPosition() {
        ...
        if (config.constraint === 'vertical') {
            this.handleVerticalMovement(x, y);
        } else if (config.constraint === 'horizontal') {
            this.handleHorizontalMovement(x, y);
        } else if (config.constraint === 'corners') {
            this.handleFreeMovement(x, y);
        } else {
            this.handleFreeMovement(x, y);
        }
    ...
    handleFreeMovement(mouseX, mouseY) {
        let newX = mouseX - this.dragCache.offsetX;
        let newY = mouseY - this.dragCache.offsetY;
        
        newX = Math.max(0, Math.min(newX, this.dragCache.maxX));
        newY = Math.max(0, Math.min(newY, this.dragCache.maxY));
    ...
    handleVerticalMovement(mouseX, mouseY) {
        let newY = mouseY - this.dragCache.offsetY;
        newY = Math.max(0, Math.min(newY, this.dragCache.maxY));
        
        if (this.activeElement.style.position != 'fixed') {
             this.activeElement.style.left = this.dragCache.startLeft + 'px';
        }
    ...
    handleHorizontalMovement(mouseX, mouseY) {
        let newX = mouseX - this.dragCache.offsetX;
        newX = Math.max(0, Math.min(newX, this.dragCache.maxX));
        
        if (this.activeElement.style.position != 'fixed') {
            this.activeElement.style.top = this.dragCache.startTop + 'px';
        }
    ...
    handleMouseUp(e) {
        ...
        if (this.activeElement.id && !e.simulated) {
            sessionStorage.setItem("ktwp-de-elem-" + this.activeElement.id + "-x", this.activeElement.style.left);
            sessionStorage.setItem("ktwp-de-elem-" + this.activeElement.id + "-y", this.activeElement.style.top);
            sessionStorage.setItem("ktwp-de-elem-" + this.activeElement.id + "-r", this.activeElement.style.right);
            sessionStorage.setItem("ktwp-de-elem-" + this.activeElement.id + "-b", this.activeElement.style.bottom);
        ...
        const event = new CustomEvent('draggableElementMoved', {
            detail: {
                element: element,
                position: { x: rectNew.left, y: rectNew.top },
                constraint: element.draggableConfig.constraint
            }
        });
        document.dispatchEvent(event);
